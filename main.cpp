...
void benchmark(const std::string &inputFile, const std::string &queryFile, const std::string &resultFile)
{
    ...
    auto c_dur = std::chrono::duration_cast<std::chrono::milliseconds>(end - begin).count();

    // query
    begin = std::chrono::steady_clock::now();
    processQueries(trie, queryFile, resultFile);
    end = std::chrono::steady_clock::now();
    auto q_dur = std::chrono::duration_cast<std::chrono::milliseconds>(end - begin).count();
    
    // output
    std::cout << "RESULT name=fahri_taban trie_construction_time=" << c_dur
              << " trie_construction_memory=" << 0
              << " query_time=" << q_dur << "\n";
